...
27 PRINTCHR$(18)"SNAKE COPYRIGHT (C) 2025 DAVERVW     ";CHR$(156);" ";
...
76 RT$=CHR$(29)
77 DN$=CHR$(17)
78 GOSUB500:GOTO 130
80 :
100 X=X+DX:Y=Y+DY
110 IF X<0 OR Y<0 OR X>=W OR Y>(T-2) THEN POKE S+S%(R),D:GOTO200
111 N=X+Y*W
112 IF PEEK(S+N)=F THEN GOSUB 300:R=R+1:GOSUB 400:R=R-1:CS=PEEK(C+N):GOTO 125
...
134 J=0:TM=TI
135 DT=TI-TM
136 SYSML:P=PEEK(JR):IFP<>0THENJ=P
137 IFDT<SDTHENIFDT>=0THEN135
140 IFJ<>0THEN142
141 GETK$:IFK$=""THEN80
142 IFJ=1THENIFDX<>0ORDY<>1THENDX=0:DY=-1
144 IFJ=2THENIFDX<>0ORDY<>-1THENDX=0:DY=1
145 IFJ=8THENIFDX<>-1ORDY<>0THENDX=1:DY=0
146 IFJ=4THENIFDX<>1ORDY<>0THENDX=-1:DY=0
...
210 GETK$:SYSML:IFK$=""AND(PEEK(JR)AND64)=0THEN210
220 RUN
300 J=INT(RND(1)*(T-1)*W)
301 LN=R+1-L:IFLN<0THENLN=LN+SZ+1
302 IF(LN/2)=INT(LN/2)ANDSD>1THENSD=SD-1
310 IFPEEK(S+J)<>32THEN300
321 C%=RND(1)*32:IFC%>15THENC%=C%+48
322 IFC%=PEEK(C+N)THEN321
323 IFC%=PEEK(C+S%(L))THEN321
324 IFC%=PEEK(C+S%(R))THEN321
328 POKEC+J,C%
329 POKES+J,F
330 RETURN
...
460 RETURN
500 DATA 78,A9,FF,8D,30,FD,A9,FB
510 DATA 8D,08,FF,AD,08,FF,49,4F
520 DATA 29,4F,8D,F2,03,A9,FF,8D
530 DATA 08,FF,58,60,--
540 :
550 ML=DEC("333"):I=0
560 READ A$:IF A$<>"--" THEN POKE ML+I,DEC(A$):I=I+1:GOTO 560
570 JR=DEC("3F2")
580 RETURN
